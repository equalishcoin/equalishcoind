...
Run with the `-testnet` option to run with "play equalishcoins" on the test network, if you
...
EqualishCoin is a multi-threaded application, and deadlocks or other
...
However, in open source software such as EqualishCoin, where everyone uses
...
pay attention to for reviewers of EqualishCoin code.

General EqualishCoin
----------------------
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers.
  ...
  - *Rationale*: EqualishCoin uses tinyformat, which is type safe. Leave them out to avoid confusion
...
Some of these are maintained by active developers of EqualishCoin, in which case
...
an EqualishCoin subtree, so that they can be integrated quickly. Cosmetic changes
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `equalishcoin-cli` and the GUI debug console use this table to determine how to
...
- Wallet RPCs call BlockUntilSyncedToCurrentChain to maintain consistency with
  ...
  - *Rationale*: In previous versions of EqualishCoin, the wallet was always
