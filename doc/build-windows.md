...
Below are some notes on how to build EqualishCoin for Windows.

The options known to work for building EqualishCoin on Windows are:
...
Note that for WSL the EqualishCoin source path MUST be somewhere in the default mount file system, for
example /usr/src/equalishcoin, AND not under /mnt/d/. If this is not the case the dependency autoconf scripts will fail.
...
way. This will install to `c:\workspace\equalishcoin`, for example:

    make install DESTDIR=/mnt/c/workspace/equalishcoin
