...
The EqualishCoin daemon can be configured to act as a trusted "border
router", implementing the equalishcoin wire protocol and relay, making
...
The ZeroMQ feature in EqualishCoin requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building equalishcoind:
...
For instance:
    ...
    $ equalishcoind -zmqpubhashtx=tcp://127.0.0.1:28332 \
...
terminator). These options can also be provided in equalishcoin.conf.
...
From the perspective of equalishcoind, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into equalishcoind directly. Furthermore, no information is
...
using. Equalishcoind appends an up-counting sequence number to each
