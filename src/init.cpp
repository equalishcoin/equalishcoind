 ...
 */
static const char* BITCOIN_PID_FILENAME = "equalishcoind.pid";
...
void SetupServerArgs(ArgsManager& argsman)
{
    ...
    argsman.AddArg("-gbtallowdisconnected", strprintf("Allow getblocktemplate when the node has no connected peers (default: %u)", false), ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY, OptionsCategory::RPC);
...
{
    // Make sure only a single EqualishCoin process is using the data directory.
...
bool AppInitMain(NodeContext& node, interfaces::BlockAndHeaderTipInfo* tip_info)
{
    ...
    if (args.IsArgSet("-datadir") && !args.GetPathArg("-datadir").is_absolute()) {
        LogPrintf("Warning: relative datadir option '%s' specified, which will be interpreted relative to the " /* Continued */
                  "current working directory '%s'. This is fragile, because if equalishcoin is started in the future "
                  "from a different location, it will be unable to locate the current data files. There could "
                  "also be data loss if equalishcoin is started while in a temporary directory.\n",
