...
bool CWallet::CreateCoinStake(ChainstateManager& chainman, const CWallet* pwallet, unsigned int nBits, int64_t nSearchInterval, CMutableTransaction& txNew, CTxDestination destination)
{
    ...
    // Calculate coin age reward
    {
        ...
        const CAmount reward_limit = GetProofOfStakeReward(nCoinAge, txNew.nTime, chainman.ActiveChain().Tip()->nMoneySupply);
        const CAmount nReward = IsIssuanceSplitEnabled(params) ? SplitIssuanceBudget(params, reward_limit).pos_budget : reward_limit;
